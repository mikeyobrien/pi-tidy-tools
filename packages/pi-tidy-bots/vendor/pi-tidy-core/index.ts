...
export function fitLine(line: string, width: number): string {
  return visibleWidth(line) <= Math.max(1, width)
    ? line
...
export function formatCount(value: number): string {
  ...
  if (value < 1_000_000)
    return `${Number((value / 1_000).toFixed(value < 10_000 ? 1 : 0))}k`;
...
function errorSummary(result: unknown): string {
  const value = result as
    | { error?: unknown; message?: unknown; details?: { error?: unknown } }
    | undefined;
  const text =
    resultText(result) ||
...
function resultSummary(
  ...
  if (name === "edit") {
    const diff =
      (result as { details?: { diff?: string } } | undefined)?.details?.diff ??
      "";
    let additions = 0,
      deletions = 0;
  ...
  if (name === "grep") {
    const matchLines = text.trim().startsWith("No matches found")
      ? []
      : text
          .split("\n")
...
export function summarizeToolActivity(
  ...
  const summary =
    state === "running"
      ? ""
      : ` → ${state === "error" ? (name === "bash" ? `error in ${formatElapsed(elapsedMs)}` : errorSummary(result)) : resultSummary(name, args, result, elapsedMs)}`;
...
export function buildToolActivityBlock(
  ...
  const headline = oneLine(
    typeof reasoning === "string" && reasoning.trim() ? reasoning : target
  );
  const mark =
    state === "running"
      ? `${DIM}·${RESET}`
  ...
  const summary =
    state === "running"
      ...
      : state === "error"
        ? name === "bash"
          ? `${RED}error${RESET} ${DIM}in ${formatElapsed(elapsedMs)}${RESET}`
          : `${RED}${errorSummary(result)}${RESET}`
