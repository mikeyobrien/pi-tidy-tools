...
import {
  ...
  readPackageVersionForEntry,
  resolveRunningPiAliases,
} from "../pi-fff/loader.js";
...
function grepTool(overrides: Record<string, unknown> = {}) {
  return {
    name: "grep",
    label: "FFF grep",
    description: "grep metadata",
    promptSnippet: "changed metadata",
    parameters: {
      type: "object",
      properties: {
        pattern: { type: "string" },
...
async function fixture(
  options: {
    ...
    userVersion?: string;
    projectIdentity?: "pi-fff" | "@ff-labs/pi-fff";
    userIdentity?: "pi-fff" | "@ff-labs/pi-fff";
    projectFactory?: ReturnType<typeof baselineFactory>;
    userFactory?: ReturnType<typeof baselineFactory>;
    projectManifest?: Record<string, unknown>;
    userManifest?: Record<string, unknown>;
    projectLock?: Record<string, unknown>;
    userLock?: Record<string, unknown>;
  } = {}
) {
  ...
  const writeScope = async (
    ...
    entry: unknown,
    version: string,
    supplied: ReturnType<typeof baselineFactory> | undefined,
    manifest: Record<string, unknown> | undefined,
    lock: Record<string, unknown> | undefined,
    identity: "pi-fff" | "@ff-labs/pi-fff"
  ) => {
    const settings =
      scope === "project"
    ...
    const managedRoot =
      scope === "project" ? join(cwd, ".pi", "npm") : join(agentDir, "npm");
    ...
    factories.set(
      await realpath(join(packageRoot, "index.ts")),
      supplied ?? baselineFactory()
    );
  };
  if (options.projectEntry !== undefined)
    await writeScope(
      "project",
      options.projectEntry,
      options.projectVersion ?? "0.1.12",
      options.projectFactory,
      options.projectManifest,
      options.projectLock,
      options.projectIdentity ?? "pi-fff"
    );
  if (options.userEntry !== undefined)
    await writeScope(
      "user",
      options.userEntry,
      options.userVersion ?? "0.1.12",
      options.userFactory,
      options.userManifest,
      options.userLock,
      options.userIdentity ?? "pi-fff"
    );
...
async function buildFrom(
  ...
  return buildPiFffRegistrationPlan({
    cwd: f.cwd,
    agentDir: f.agentDir,
    piVersion,
    api,
...
function expectCode(
  result: Awaited<ReturnType<typeof buildPiFffRegistrationPlan>>,
  code: PiFffDiagnostic["code"]
) {
  assert.equal(result.ok, false);
  if (!result.ok) assert.equal(result.diagnostic.code, code);
}

const filtered = (version = "0.1.12") => ({
  source: `npm:pi-fff@${version}`,
  extensions: [],
});
...
const lockFor = (
  ...
  name: "managed-pi-packages",
  lockfileVersion: 3,
  packages: {
    "node_modules/pi-fff": {
      version,
      resolved: `https://registry.npmjs.org/pi-fff/-/pi-fff-${version}.tgz`,
      integrity: INTEGRITY,
      ...overrides,
    },
  },
});
const scopedFiltered = (version = "0.9.6") => ({
  source: `npm:@ff-labs/pi-fff@${version}`,
  extensions: [],
});
const scopedLock = (version = "0.9.6") => ({
  ...
  packages: {
    "node_modules/@ff-labs/pi-fff": {
      version,
      resolved: `https://registry.npmjs.org/@ff-labs/pi-fff/-/pi-fff-${version}.tgz`,
      integrity: INTEGRITY,
    },
  },
  dependencies: {
...
function registerScopedSurface(
  pi: any,
  tools: any[],
  extra?: (pi: any) => void,
  flags: readonly (readonly [string, "string" | "boolean"])[] = [
    ["fff-mode", "string"],
    ["fff-frecency-db", "string"],
    ["fff-history-db", "string"],
    ["fff-enable-root-scan", "boolean"],
  ]
) {
  for (const [name, type] of flags) pi.registerFlag(name, { type });
  for (const tool of tools) pi.registerTool(tool);
  extra?.(pi);
  for (const name of ["fff-mode", "fff-health", "fff-rescan"])
...
function scopedFactory(
  mode: "tools-and-ui" | "tools-only" | "override" = "tools-and-ui"
) {
  ...
  source.factory = (pi: any) => {
    assert.equal(
      pi.getFlag("fff-mode"),
      mode === "tools-and-ui" ? undefined : mode
    );
    for (const [name, type] of [
      ...
      pi.registerFlag(name, { type });
    const names =
      mode === "override"
        ? ["grep", "find", "multi_grep"]
        : ["ffgrep", "fffind", "fff-multi-grep"];
    for (const name of names)
      pi.registerTool(scopedTool(name, { renderCall, renderResult }));
    let append = pi.appendEntry;
    pi.registerCommand("fff-mode", {
      handler() {
        append("fff-mode", { mode: "tools-only" });
      },
    });
...
test("scoped profile captures raw pair and substitutes public tidy names in trace order", async () => {
  ...
  const f = await fixture({
    userEntry: scopedFiltered(),
    userVersion: "0.9.6",
    userIdentity: "@ff-labs/pi-fff",
    userFactory: scoped.source,
    userLock: scopedLock(),
  });
  const real = apiRecorder();
  real.api.getFlag = () => undefined;
  let appended = 0;
  real.api.appendEntry = () => {
    appended++;
  };
  try {
    ...
    if (result.plan.profile !== "scoped") return;
    ...
    const recordedMode = result.plan.trace.find(
      (call) => call.method === "registerCommand" && call.args[0] === "fff-mode"
    )?.args[1] as any;
    assert.throws(
      () => recordedMode.handler(),
      /registration-time action appendEntry is unsafe/
    );
    const composites = createPiFffComposites(result.plan, {
      mode: "default",
      reasoningGuideline: "State the goal.",
    });
    assert.equal(
      replayPiFffRegistrationPlan(result.plan, real.api, composites).ok,
      true
    );
    const tools = real.calls
      .filter((call) => call.method === "registerTool")
      .map((call) => call.args[0] as any);
    assert.deepEqual(
      tools.map((tool) => tool.name),
      ["grep", "find", "fff-multi-grep"]
    );
    assert.equal(
      tools.filter((tool) => ["ffgrep", "fffind"].includes(tool.name)).length,
      0
    );
    assert.equal(tools[0], composites.grep);
    assert.equal(tools[1], composites.find);
    assert.equal(composites.grep.renderCall, scoped.renderCall);
    assert.equal(composites.grep.renderResult, scoped.renderResult);
    const mode = real.calls.find(
      (call) => call.method === "registerCommand" && call.args[0] === "fff-mode"
    )?.args[1] as any;
    mode.handler();
...
test("scoped tools-only trace captures the same explicit pair", async () => {
  ...
  const f = await fixture({
    ...
    userVersion: "0.9.6",
    userIdentity: "@ff-labs/pi-fff",
    userFactory: toolsOnly.source,
  });
  const real = apiRecorder();
  real.api.getFlag = () => "tools-only";
  try {
    const result = await buildFrom(f, real.api);
    assert.equal(result.ok, true);
    if (!result.ok) return;
    ...
    assert.deepEqual(
      result.plan.trace
        .filter((call) => call.method === "registerTool")
        .map((call) => (call.args[0] as any).name),
      ["ffgrep", "fffind", "fff-multi-grep"]
    );
  } finally {
    await rm(f.root, { recursive: true, force: true });
...
test("scoped 0.6.0 schema and newer optional additions both validate", async () => {
  ...
  const source = baselineFactory();
  source.factory = (pi: any) =>
    registerScopedSurface(pi, [grep, find], undefined, [
      ["fff-mode", "string"],
      ["fff-frecency-db", "string"],
      ["fff-history-db", "string"],
    ]);
  const f = await fixture({
    userEntry: scopedFiltered("0.6.0"),
    userVersion: "0.6.0",
    userIdentity: "@ff-labs/pi-fff",
    userFactory: source,
  });
  try {
    ...
    assert.equal(result.ok, true);
    if (!result.ok || result.plan.profile !== "scoped") return;
    assert.equal(
      result.plan.captures.grep.parameters.properties.literal.type,
      "boolean"
    );
    assert.equal(
      result.plan.captures.find.parameters.properties.cursor,
      undefined
    );
  } finally {
    await rm(f.root, { recursive: true, force: true });
...
test("scoped 0.9.5+ requires the fourth current flag", async () => {
  const source = baselineFactory();
  source.factory = (pi: any) =>
    registerScopedSurface(
      pi,
      [scopedTool("ffgrep"), scopedTool("fffind")],
      undefined,
      [
        ["fff-mode", "string"],
        ["fff-frecency-db", "string"],
  ...
  const f = await fixture({
    userEntry: scopedFiltered("0.9.5"),
    userVersion: "0.9.5",
    userIdentity: "@ff-labs/pi-fff",
    userFactory: source,
  });
  try {
    expectCode(await buildFrom(f), "PIFFF_SURFACE_BREAKING");
  } finally {
    await rm(f.root, { recursive: true, force: true });
  }
});
...
test("scoped composites preserve metadata, schemas by mode, and exact execution semantics", async () => {
  ...
  const grep = scopedTool("ffgrep", {
    marker: Symbol.for("grep"),
    execute(this: unknown, ...args: unknown[]) {
      observed = [this, ...args];
      (args[3] as Function)(textResult("update"));
      return settled;
    },
  });
  const find = scopedTool("fffind");
  const source = baselineFactory();
  source.factory = (pi: any) => registerScopedSurface(pi, [grep, find]);
  const f = await fixture({
    userEntry: scopedFiltered(),
    userVersion: "0.9.6",
    userIdentity: "@ff-labs/pi-fff",
  ...
  try {
    ...
    assert.equal(result.ok, true);
    if (!result.ok || result.plan.profile !== "scoped") return;
    assert.equal(result.plan.captures.grep, grep);
    assert.equal(result.plan.captures.find, find);
    const resultMode = createPiFffComposites(result.plan, {
      mode: "result",
      reasoningGuideline: "reason",
    });
    ...
    assert.equal(resultMode.grep.name, "grep");
    assert.equal(resultMode.find.name, "find");
    const defaults = createPiFffComposites(result.plan, {
      mode: "default",
      reasoningGuideline: "reason",
    });
    assert.ok(
      defaults.grep.parameters.properties.reasoning,
      "reasoning present"
    );
    assert.ok(
      !defaults.grep.parameters.required?.includes("reasoning"),
      "reasoning optional"
    );
    assert.deepEqual(defaults.grep.promptGuidelines, [
      ...grep.promptGuidelines,
      "reason",
    ]);
    const params = {
      reasoning: "search index",
      pattern: "needle",
      optional: 1,
      nested: { same: true },
    };
    ...
    assert.equal(
      await defaults.grep.execute("id", params, signal, onUpdate, context),
      settled
    );
    assert.equal(observed[0], grep);
    ...
    assert.equal((observed[2] as any).nested, params.nested);
    assert.equal(observed[3], signal);
    assert.equal(observed[4], onUpdate);
    ...
    assert.throws(
      () =>
        defaults.find.execute(
          "id",
          { pattern: "x", reasoning: "find x" },
          signal,
          update,
          context
        ),
...
test("scoped capture failures and target conflicts fail before replay", async (t) => {
  const cases: Array<[string, any[], Record<string, unknown> | undefined]> = [
    ...
    [
      "duplicate ffgrep",
      [scopedTool("ffgrep"), scopedTool("ffgrep"), scopedTool("fffind")],
      undefined,
    ],
    [
      "duplicate fffind",
      [scopedTool("ffgrep"), scopedTool("fffind"), scopedTool("fffind")],
      undefined,
    ],
    [
      "malformed definition",
      [
        scopedTool("ffgrep", { parameters: { type: "string" } }),
        scopedTool("fffind"),
      ],
      undefined,
    ],
    ...
    [
      "narrowed primitive property",
      [
        scopedTool("ffgrep", {
          parameters: {
            ...scopedTool("ffgrep").parameters,
            properties: {
              ...scopedTool("ffgrep").parameters.properties,
              path: { type: "string", enum: ["src"] },
            },
          },
        }),
        scopedTool("fffind"),
    ...
    [
      "malformed prompt guidance",
      [
        scopedTool("ffgrep", { promptGuidelines: ["valid", 42] }),
        scopedTool("fffind"),
      ],
      undefined,
    ],
    ...
    [
      "target grep conflict",
      [scopedTool("ffgrep"), scopedTool("fffind")],
      { tools: ["grep"] },
    ],
    [
      "target find conflict",
      [scopedTool("ffgrep"), scopedTool("fffind")],
      { tools: ["find"] },
    ],
  ];
  for (const [name, tools, conflicts] of cases)
    await t.test(name, async () => {
      const source = baselineFactory();
      source.factory = (pi: any) => registerScopedSurface(pi, tools);
      const f = await fixture({
        userEntry: scopedFiltered(),
        userVersion: "0.9.6",
        userIdentity: "@ff-labs/pi-fff",
      ...
      const real = apiRecorder();
      try {
        const result = await buildPiFffRegistrationPlan({
          cwd: f.cwd,
          agentDir: f.agentDir,
          piVersion: "0.80.6",
          api: real.api,
...
test("scoped production static conflicts allow substitutions while caller targets fail", async (t) => {
  assert.deepEqual(TIDY_PI_FFF_CONFLICTS, {
    tools: ["write", "edit", "bash", "ls"],
    commands: ["tidy", "diff"],
    shortcuts: ["ctrl+shift+o"],
    messageRenderers: ["minimal-turn-diff"],
  });
  const source = baselineFactory();
  source.factory = (pi: any) =>
    registerScopedSurface(pi, [scopedTool("ffgrep"), scopedTool("fffind")]);
...
test("scoped compatible additions replay once around substituted slots", async () => {
  const before = () => textResult("before");
  const after = () => {};
  const source = baselineFactory();
  source.factory = (pi: any) =>
    registerScopedSurface(
      pi,
      [
        scopedTool("future-before", { execute: before }),
        scopedTool("ffgrep"),
        scopedTool("future-middle"),
        scopedTool("fffind"),
      ],
      (recording) => recording.registerShortcut("ctrl+f", { handler: after })
    );
  const f = await fixture({
    userEntry: scopedFiltered("1.0.0"),
    userVersion: "1.0.0",
    userIdentity: "@ff-labs/pi-fff",
    userFactory: source,
  });
  ...
  try {
    ...
    assert.equal((real.calls[4]!.args[0] as any).execute, before);
    assert.equal((real.calls[8]!.args[1] as any).handler, after);
  } finally {
    await rm(f.root, { recursive: true, force: true });
  }
});
...
test("scoped override mode and mixed identities fail before replay", async () => {
  ...
  const f = await fixture({
    ...
    userVersion: "0.9.6",
    userIdentity: "@ff-labs/pi-fff",
    userFactory: override.source,
  });
  const real = apiRecorder();
  real.api.getFlag = () => "override";
  try {
    expectCode(await buildFrom(f, real.api), "PIFFF_SURFACE_BREAKING");
    assert.equal(real.calls.length, 0);
    await writeFile(
      join(f.agentDir, "settings.json"),
      JSON.stringify({ packages: [scopedFiltered(), filtered()] })
    );
...
test("managed discovery selects canonical project root and never falls back", async () => {
  ...
  const f = await fixture({
    projectEntry: { source: "npm:pi-fff@0.1.12", extensions: ["./index.ts"] },
    userEntry: filtered(),
    projectFactory: invalidProject,
  });
  try {
    const result = await buildFrom(f);
...
test("lifecycle preflight can fully validate selected and shadowed participants independently", async () => {
  ...
  const f = await fixture({
    projectEntry: entryForPreflight(),
    userEntry: entryForPreflight(),
    projectFactory: project,
    userFactory: user,
  });
  try {
    const projectResult = await buildPiFffRegistrationPlan({
      cwd: f.cwd,
      agentDir: f.agentDir,
      piVersion: "0.80.6",
      api: apiRecorder().api,
      loader: f.loader,
      selection: { scope: "project", entry: entryForPreflight() },
    });
    const userResult = await buildPiFffRegistrationPlan({
      cwd: f.cwd,
      agentDir: f.agentDir,
      piVersion: "0.80.6",
      api: apiRecorder().api,
      loader: f.loader,
      selection: { scope: "user", entry: entryForPreflight() },
    });
...
test("managed discovery ignores non-npm scope and rejects string filters", async () => {
  const ignored = await fixture({
    projectEntry: "git:github.com/example/pi-fff",
    userEntry: filtered(),
  });
...
test("lock identity, version, resolved, and integrity are checked offline", async (t) => {
  await t.test(
    "matching local lock continues with registry-unverified information",
    async () => {
      ...
      try {
        ...
        assert.equal(
          result.plan.diagnostics[0]?.code,
        ...
        try {
          const available = await buildFrom(
            verified,
            apiRecorder().api,
            "0.80.6",
            INTEGRITY
          ...
          await rm(verified.root, { recursive: true, force: true });
        }
      } finally {
        await rm(f.root, { recursive: true, force: true });
      }
  ...
  for (const [name, lock, registry] of [
    ...
    [
      ...
      lockFor("0.1.12", {
        resolved: "https://registry.npmjs.org/other/-/other-0.1.12.tgz",
      }),
      undefined,
    ],
    [
      "malformed integrity",
      lockFor("0.1.12", { integrity: "not-sri" }),
      undefined,
    ],
    ...
    await t.test(name, async () => {
      ...
      try {
        expectCode(
          await buildFrom(f, apiRecorder().api, "0.80.6", registry),
  ...
  await t.test("missing integrity remains compatible", async () => {
    const f = await fixture({
      userEntry: filtered(),
      userLock: lockFor("0.1.12", { integrity: undefined }),
    });
    try {
      const result = await buildFrom(f);
...
test("version floors have no upper bound and status is explicit", async () => {
  ...
  try {
    ...
    assert.equal(
      forward.ok && forward.plan.status,
      "forward-compatible/unverified"
    );
    expectCode(
      await buildFrom(baseline, apiRecorder().api, "0.80.5"),
      "PIFFF_BELOW_MINIMUM"
    );
    const oldFff = await fixture({
      userEntry: filtered("0.1.11"),
      userVersion: "0.1.11",
    });
    try {
      expectCode(await buildFrom(oldFff), "PIFFF_BELOW_MINIMUM");
    } finally {
      await rm(oldFff.root, { recursive: true, force: true });
    }
  } finally {
...
test("scoped profile enforces 0.6.0 floor independently", async () => {
  const old = await fixture({
    userEntry: scopedFiltered("0.5.9"),
    userVersion: "0.5.9",
    userIdentity: "@ff-labs/pi-fff",
    userFactory: scopedFactory().source,
  });
  const floor = await fixture({
    userEntry: scopedFiltered("0.6.0"),
    userVersion: "0.6.0",
    userIdentity: "@ff-labs/pi-fff",
    userFactory: scopedFactory().source,
  ...
  try {
    ...
    const accepted = await buildFrom(floor);
    assert.equal(accepted.ok, true);
    assert.equal(
      accepted.ok && accepted.plan.status,
      "forward-compatible/unverified"
    );
  } finally {
    await rm(old.root, { recursive: true, force: true });
    await rm(floor.root, { recursive: true, force: true });
  }
});
...
test("SemVer rejects malformed cores and honors prerelease precedence", async () => {
  ...
  const leadingFff = await fixture({
    userEntry: filtered("01.1.12"),
    userVersion: "01.1.12",
  });
  const prereleaseFloor = await fixture({
    userEntry: filtered("0.1.12-rc.1"),
    userVersion: "0.1.12-rc.1",
  });
  const futurePrerelease = await fixture({
    userEntry: filtered("0.1.13-alpha.2"),
    userVersion: "0.1.13-alpha.2",
  });
  try {
    expectCode(
      await buildFrom(malformedPi, apiRecorder().api, "01.80.6"),
      "PIFFF_BELOW_MINIMUM"
    );
    ...
    assert.equal(
      future.ok && future.plan.status,
      "forward-compatible/unverified"
    );
    assert.equal(
      future.ok &&
        future.plan.diagnostics.some(
          (item) => item.code === "PIFFF_FORWARD_UNVERIFIED"
        ),
      true
    );
  } finally {
...
test("running Pi Jiti aliases legacy peers and shared TypeBox to one identity", async () => {
  ...
  try {
    ...
    assert.throws(
      () => readPackageVersionForEntry(join(root, "missing", "entry.js")),
      /running Pi package root is unavailable/
    );
  } finally {
...
test("running Pi resolution handles host manifests and rejects missing Jiti capability", async () => {
  ...
  try {
    ...
    await writeFile(
      join(jitiRoot, "package.json"),
      JSON.stringify({
        name: "jiti",
        exports: {
          "./package.json": "./package.json",
          "./static": { import: "./static.mjs" },
        },
      })
    );
    await writeFile(
      join(jitiRoot, "static.mjs"),
    ...
    await assert.rejects(
      missingFactory.loader.load(
        join(root, "entry.ts"),
        missingFactory.aliases
      ),
      /running Pi Jiti factory is unavailable/
    );
    ...
    assert.throws(
      resolveRunningPiAliases,
      /running Pi Jiti static export is unavailable/
    );
    await writeFile(
      join(root, "package.json"),
      JSON.stringify({ name: "not-pi" })
    );
    resolveRunningPiAliases();
    ...
    assert.throws(
      () => readPackageVersionForEntry(join(root, "index.js")),
...
test("bundled Pi runtime resolves aliases from the process entry when the bare specifier is unresolvable", async () => {
  const root = await realpath(
    await mkdtemp(join(tmpdir(), "tidy-pi-fff-bundled-"))
  );
  const piRoot = join(
    root,
    "node_modules",
  ...
  try {
    ...
    await writeFile(
      join(piRoot, "node_modules", "@earendil-works", "pi-tui", "package.json"),
      JSON.stringify({ name: "@earendil-works/pi-tui", main: "index.js" })
    );
    await writeFile(
      join(piRoot, "node_modules", "@earendil-works", "pi-tui", "index.js"),
      "export {};\n"
    );
    await writeFile(
      join(piRoot, "node_modules", "typebox", "package.json"),
      JSON.stringify({ name: "typebox", main: "index.js" })
    );
    for (const file of ["index.js", "compile.js", "value.js"])
      await writeFile(
        join(piRoot, "node_modules", "typebox", file),
        "export {};\n"
      );
    await writeFile(
      join(jitiRoot, "package.json"),
    ...
    assert.equal(
      direct.aliases["@earendil-works/pi-coding-agent"],
      direct.aliases.codingAgent
    );
...
test("unlocatable running Pi fails closed with PIFFF_PI_ROOT_UNAVAILABLE", async () => {
  const root = await realpath(
    await mkdtemp(join(tmpdir(), "tidy-pi-fff-orphan-"))
  );
  ...
  try {
    await writeFile(
      join(root, "orphan-run.js"),
      "// not inside any Pi package\n"
    );
    process.argv[1] = join(root, "orphan-run.js");
    assert.throws(
      () => resolveRunningPiAliases({ resolveModule: unresolvable }),
      /running Pi package root is unavailable/
    );
    const result = await buildPiFffRegistrationPlan({
      cwd: f.cwd,
      ...
      piVersion: "0.84.3",
      api: apiRecorder().api,
      resolvePiModule: unresolvable,
    });
    expectCode(result, "PIFFF_PI_ROOT_UNAVAILABLE");
    assert.match(
      !result.ok ? result.diagnostic.summary : "",
      /running Pi installation could not be located/
    );
  } finally {
    process.argv[1] = previousArgv;
    await rm(root, { recursive: true, force: true });
...
test("factory is loaded and invoked once while real registrations remain zero", async () => {
  ...
  try {
    ...
    assert.equal(real.calls.length, 0);
    assert.deepEqual(
      result.ok && result.plan.trace.map((call) => call.method),
      [
        "registerTool",
        "registerTool",
        "registerTool",
...
test("async factory and result are awaited exactly once", async () => {
  ...
  try {
    ...
    const tools = createPiFffComposites(result.plan, {
      mode: "result",
      reasoningGuideline: "reason",
    });
    assert.equal(
      await tools.read.execute("id", { path: "x" }, undefined, undefined, {}),
      expected
    );
  } finally {
    await rm(f.root, { recursive: true, force: true });
...
test("compatible forward additions preserve metadata, tool schema, identities, and order on replay", async () => {
  ...
  try {
    ...
    const replay = replayPiFffRegistrationPlan(
      result.plan,
      real.api,
      composites
    );
    ...
    assert.deepEqual(
      real.calls.map((call) => call.method),
      [
        "registerTool",
        "registerTool",
        "on",
    ...
    assert.equal(
      (real.calls[3]?.args[0] as any).parameters.properties.query.type,
      "string"
    );
...
test("breaking surface matrix fails closed with stable diagnostics", async (t) => {
  const cases: Array<[string, (pi: any) => void]> = [
    ...
    [
      "noncallable captured executor",
      (pi) => {
        pi.registerTool(readTool({ execute: "no" }));
        pi.registerTool(grepTool());
        pi.on("session_start", () => {});
        pi.on("session_shutdown", () => {});
      },
    ...
    [
      "source-owned captured reasoning",
      (pi) => {
        pi.registerTool(
          readTool({
            parameters: {
    ...
    [
      "malformed forwarded parameters",
      (pi) => {
        pi.registerTool(readTool());
        pi.registerTool(grepTool());
        pi.registerTool({
    ...
    [
      "noncallable forwarded executor",
      (pi) => {
        pi.registerTool(readTool());
        pi.registerTool(grepTool());
        pi.registerTool({
    ...
    [
      "overlapping tidy tool",
      (pi) => {
        pi.registerTool(readTool());
        pi.registerTool(grepTool());
        pi.registerTool({ ...readTool(), name: "write" });
        pi.on("session_start", () => {});
        pi.on("session_shutdown", () => {});
      },
    ],
    ...
    [
      ...
      (pi) => {
        void pi.registerWidget;
      },
    ],
  ];
  for (const [name, customFactory] of cases)
    await t.test(name, async () => {
      const source = baselineFactory();
      source.factory = customFactory;
      const f = await fixture({ userEntry: filtered(), userFactory: source });
      const real = apiRecorder();
      try {
        expectCode(await buildFrom(f, real.api), "PIFFF_SURFACE_BREAKING");
...
test("semantic singleton primitive schema forms pass without accepting broadened unions", async () => {
  ...
  const accepted = await fixture({
    userEntry: filtered(),
    userFactory: equivalent,
  });
  const rejected = await fixture({
    userEntry: filtered(),
    userFactory: broadened,
  });
  try {
...
test("known registration methods validate and preserve nonconflicting additions", async () => {
  ...
  try {
    ...
    if (!result.ok) return;
    assert.deepEqual(
      result.plan.trace.slice(-5).map((call) => call.method),
      [
        "registerShortcut",
        "registerFlag",
        "registerMessageRenderer",
...
test("known registration conflicts fail before replay", async (t) => {
  for (const [name, command, conflicts] of [
    ...
    await t.test(name, async () => {
      const source = baselineFactory((pi) =>
        pi.registerCommand(command, { handler() {} })
      );
      const f = await fixture({ userEntry: filtered(), userFactory: source });
      const real = apiRecorder();
      try {
...
test("load, factory, and capability failures are sanitized and side-effect free", async () => {
  const f = await fixture({ userEntry: filtered() });
  try {
    const missingApi = apiRecorder();
    delete missingApi.api.registerFlag;
    expectCode(await buildFrom(f, missingApi.api), "PIFFF_CAPABILITY_MISSING");
    const load = await buildPiFffRegistrationPlan({
      cwd: f.cwd,
      agentDir: f.agentDir,
      piVersion: "0.80.6",
      api: apiRecorder().api,
    ...
    const throwing = baselineFactory();
    throwing.factory = (pi: any) => {
      pi.registerTool(readTool());
      throw new Error("factory secret\nstack");
    };
    const ff = await fixture({ userEntry: filtered(), userFactory: throwing });
    try {
      expectCode(await buildFrom(ff), "PIFFF_FACTORY_FAILED");
    } finally {
      await rm(ff.root, { recursive: true, force: true });
    }
  } finally {
    await rm(f.root, { recursive: true, force: true });
  }
...
test("registration-time mutators and property writes cannot escape the recorder", async (t) => {
  for (const [name, escape] of [
    [
      "message",
      (pi: any) => pi.sendMessage({ customType: "unsafe", content: "x" }),
    ],
...
test("deferred API closures bind the real receiver only after activation", async () => {
  ...
  const source = baselineFactory((pi) => {
    ...
    pi.registerCommand("deferred", {
      handler() {
        deferred({ customType: "safe", content: "x" });
      },
    });
  });
  const f = await fixture({ userEntry: filtered(), userFactory: source });
  const real = apiRecorder();
  real.api.sendMessage = function () {
    receiver = this;
  };
  try {
    const result = await buildFrom(f, real.api);
    ...
    if (!result.ok) return;
    handler =
      result.plan.trace.find(
        (call) =>
          call.method === "registerCommand" && call.args[0] === "deferred"
      )?.args[1] &&
      (
        result.plan.trace.find(
          (call) =>
...
test("composite delegation preserves receiver, five arguments, updates, results, aborts, and errors", async () => {
  ...
  try {
    ...
    const actual = await composites.read.execute(
      "id",
      params,
      signal,
      update,
    ...
    assert.equal(observed[5], context);
    const aborted = new DOMException("aborted", "AbortError");
    result.plan.captures.grep.execute = () => {
      throw aborted;
    };
    assert.throws(
      () => composites.grep.execute("id", {}, signal, update, context),
      (error) => error === aborted
    );
  } finally {
    await rm(f.root, { recursive: true, force: true });
...
test("malformed settled results fail closed without native retry", async () => {
  ...
  try {
    ...
    assert.throws(
      () => tools.read.execute("id", { path: "x" }, undefined, undefined, {}),
      (error: any) => error?.code === "PIFFF_EXEC_RESULT_INVALID"
    );
...
test("partial updates preserve callback, object identity, and source behavior naturally", async () => {
  const validPartial = {
    ...textResult("partial", { truncation: { truncated: false } }),
    custom: Symbol("kept"),
  };
  ...
  source.factory = (pi: any) => {
    pi.registerTool(
      readTool({
        execute(_id: string, _params: any, _signal: any, onUpdate: any) {
  ...
  try {
    ...
    const actual = await tools.read.execute(
      "id",
      { path: "x" },
      undefined,
      (value: unknown) => observed.push(value),
      {}
    );
    ...
    assert.equal(
      await tools.read.execute(
        ...
        { path: "x" },
        undefined,
        (value: unknown) => malformedUpdates.push(value),
        {}
      ),
      settled
    );
...
test("read and grep result families preserve native and FFF details", async () => {
  ...
  try {
    ...
    assert.equal(
      await tools.read.execute("1", {}, undefined, undefined, {}),
      families[0]
    );
    assert.equal(
      await tools.read.execute("2", {}, undefined, undefined, {}),
      families[1]
    );
    assert.equal(
      await tools.grep.execute("3", {}, undefined, undefined, {}),
      families[2]
    );
    assert.equal(
      await tools.grep.execute("4", {}, undefined, undefined, {}),
      families[3]
    );
  } finally {
    await rm(f.root, { recursive: true, force: true });
...
test("replay rejects malformed composites before any registration", async () => {
  ...
  try {
    ...
    if (!result.ok) return;
    const real = apiRecorder();
    const replay = replayPiFffRegistrationPlan(result.plan, real.api, {
      read: {} as any,
      grep: {} as any,
    });
    assert.equal(replay.ok, false);
    if (!replay.ok)
      assert.equal(replay.diagnostic.code, "PIFFF_SURFACE_BREAKING");
    assert.equal(real.calls.length, 0);
  } finally {
    await rm(f.root, { recursive: true, force: true });
  }
...
test("replay stops after an unexpected registration failure", async () => {
  ...
  try {
    ...
    const real = apiRecorder();
    let count = 0;
    real.api.registerTool = (...args: unknown[]) => {
      real.calls.push({ method: "registerTool", args });
      if (++count === 2) throw new Error("rejected");
    };
    const composites = createPiFffComposites(result.plan, {
      mode: "result",
      reasoningGuideline: "reason",
    ...
    assert.equal(replay.ok, false);
    if (!replay.ok)
      assert.equal(replay.diagnostic.code, "PIFFF_FORWARD_PARTIAL");
