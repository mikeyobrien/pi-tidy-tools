...
> {
  ...
  const identity = async (pid: number) => {
    ...
    const probe = await probeDaemonIdentity(
      ...
      resolve(dir),
      undefined,
      // Issue 154 fix: token-protected daemons 401 an unauthenticated
      // /api/version, which probe read as "not serving" and made stop/
      // restart refuse (restart became a silent no-op). Same credential the
      // daemonize parent readiness poll uses (issue 51).
      readStoredToken(resolve(dir))
    );
