...
| Continuity | `verified` | Protocol requires this when load is advertised. Fixture reload proved same thread id. Native Codex `thread/resume` across a real app-server restart is not-run. |
...
same native thread or fail closed.
