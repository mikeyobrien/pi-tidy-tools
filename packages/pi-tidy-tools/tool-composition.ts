...
export type ComposedSourceTool<T extends SourceToolDefinition> = Omit<
  T,
  "execute" | "parameters"
> & {
  parameters: any;
...
export function withReasoning(parameters: any): any {
  ...
  // Issue 61 layer 1: reasoning is optional — compliant models fill it,
  // non-compliant ones are not hard-rejected into a retry loop.
  const required = Array.from(
    new Set(
      (parameters?.required ?? []).filter((r: string) => r !== "reasoning")
    )
  );
...
export function composeSourceTool<T extends SourceToolDefinition>(
  ...
  const sourceHasReasoning =
    !!source.parameters?.properties &&
    Object.hasOwn(source.parameters.properties, "reasoning");
  if (!resultMode && sourceHasReasoning) {
    throw new Error(
      `${source.name} source schema reserves tidy-owned reasoning`
    );
  }
  const injectReasoning = !resultMode;
  const composed = {
    ...source,
    parameters: injectReasoning
      ? withReasoning(source.parameters)
      : source.parameters,
    execute(
      this: SourceToolDefinition,
      id: string,
      params: any,
      signal: any,
      onUpdate: any,
      ...
      const delegatedParams = injectReasoning
        ? stripReasoning(params).rest
        : params;
  ...
  if (!resultMode)
    composed.promptGuidelines = [
      ...(source.promptGuidelines ?? []),
      options.reasoningGuideline,
    ];
  return composed;
}
