...
const identityOf = (value: any) =>
  String(typeof value === "string" ? value : value?.source).startsWith(
    "npm:@ff-labs/pi-fff"
  )
    ? ("@ff-labs/pi-fff" as const)
    : ("pi-fff" as const);
...
async function fixture(
  options: {
    ...
    user?: unknown;
    projectMode?: number;
    symlinkProject?: boolean;
  } = {}
) {
  const root = await realpath(
    await mkdtemp(join(tmpdir(), "tidy-fff-lifecycle-"))
  );
  ...
  const paths = {
    ...
    user: join(agentDir, "settings.json"),
  };
  const writeSettings = async (
    ...
    const bytes =
      JSON.stringify(
        { theme: scope, nested: { preserved: true }, packages: [value] },
        null,
        2
      ) + "\n";
  ...
  if (options.project !== undefined)
    await writeSettings("project", options.project, options.projectMode);
  ...
  for (const scope of ["project", "user"] as const)
    if (options[scope] !== undefined) {
      ...
      await writeFile(
        ...
        JSON.stringify({
          name: identity,
          version: identity === "pi-fff" ? "0.1.12" : "0.9.6",
  ...
  const tool = (name: "read" | "grep") => ({
    ...
    parameters: {
      ...
      required: [name === "read" ? "path" : "pattern"],
    },
    execute() {
  ...
  const preflight = async (participant: PiFffLifecycleParticipant) => {
    ...
    const built = await buildPiFffRegistrationPlan({
      ...
      piVersion: "0.80.6",
      api,
      selection: { scope: participant.scope, entry: participant.managedEntry },
      loader: {
        async load() {
          return (pi: any) => {
            if (participant.profile === "legacy") {
              pi.registerTool(tool("read"));
              pi.registerTool(tool("grep"));
            } else {
  ...
  const lifecycle = (extra: Record<string, unknown> = {}) =>
    createPiFffLifecycle({ cwd, agentDir, preflight, ...extra });
...
const sidecar = (settings: string) =>
  join(dirname(settings), "pi-tidy-tools.pi-fff.json");

async function setup(
  f: Awaited<ReturnType<typeof fixture>>,
  confirm = async (_preview: PiFffLifecyclePreview) => true
) {
  let reloads = 0;
  const result = await f.lifecycle().run("setup", {
    enabled: true,
    confirm,
    reload: async () => {
      reloads++;
...
test("scoped setup and teardown preserve exact source identity", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      [
        ["project", "@ff-labs/pi-fff", "scoped"],
        ["user", "@ff-labs/pi-fff", "scoped"],
      ]
    );
    assert.deepEqual((await json(f.paths.project)).packages[0], {
      ...projectPrior,
      extensions: [],
    });
    assert.deepEqual((await json(f.paths.user)).packages[0], {
      source: userPrior,
      extensions: [],
    });
    const torn = await f
...
test("both package identities in one scope are ambiguous", async () => {
  ...
  try {
    await writeFile(
      f.paths.user,
      JSON.stringify({
        packages: ["npm:pi-fff@0.1.12", "npm:@ff-labs/pi-fff@0.9.6"],
      })
    );
    ...
    await t.test(name, async () => {
      ...
      try {
        const before = await Promise.all([
          readFile(f.paths.project),
          readFile(f.paths.user),
        ]);
        ...
        for (const result of [status, setupResult]) {
          ...
          assert.match(
            result.message,
            /one pi-fff package identity across project and user settings/i
          );
        }
...
test("setup preflights every participant before confirmation or writes and preserves exact entry fields", async () => {
  const f = await fixture({
    ...
    user: "npm:pi-fff@0.1.12",
    projectMode: 0o640,
  });
  try {
    let preview: PiFffLifecyclePreview | undefined;
    const { result, reloads } = await setup(f, async (value) => {
      preview = value;
      return true;
    });
    assert.equal(result.outcome, "setup-committed");
    ...
    assert.deepEqual(
      preview?.changes.map((change) => change.scope),
      ["project", "user"]
    );
    const project = await json(f.paths.project);
    assert.deepEqual(project.packages[0], {
      ...entry("project"),
      extensions: [],
    });
    ...
    for (const scope of ["project", "user"] as const) {
      ...
      assert.equal(journal.operation, "setup");
      assert.equal(journal.phase, "committed");
      assert.equal(journal.scope, scope);
      assert.equal(typeof journal.transactionId, "string");
      assert.equal(journal.participants.length, 2);
      assert.equal(journal.settingsPath, f.paths[scope]);
      assert.deepEqual(journal.counterpartPaths, [
        f.paths[scope === "project" ? "user" : "project"],
      ]);
  ...
  try {
    const setupResult = await f.lifecycle().run("setup", {
      ...
      reload: async () => {
        for (const scope of ["project", "user"] as const)
          assert.equal(
            (await json(sidecar(f.paths[scope]))).phase,
            "reload-pending"
          );
      },
    });
    ...
    const teardownResult = await f.lifecycle().run("teardown", {
      ...
      reload: async () => {
        for (const scope of ["project", "user"] as const) {
          ...
          assert.deepEqual(
            (await json(f.paths[scope])).packages[0],
            entry(scope)
          );
        }
      },
...
test("replacement startup inside reload finalizes setup and teardown before the old command resumes", async (t) => {
  await t.test("dual-scope setup", async () => {
    ...
    try {
      let replacement:
        | Awaited<ReturnType<ReturnType<typeof f.lifecycle>["initialize"]>>
        | undefined;
      const command = await f.lifecycle().run("setup", {
        ...
        reload: async () => {
          ...
          assert.deepEqual(
            replacement.participants?.map((participant) => participant.scope),
            ["project", "user"]
          );
          for (const scope of ["project", "user"] as const) {
            assert.deepEqual((await json(f.paths[scope])).packages[0], {
              ...entry(scope),
              extensions: [],
            });
            assert.equal(
...
test("failed setup and teardown reloads remain pending until fresh startup converges ready", async (t) => {
  await t.test("setup", async () => {
    const f = await fixture({ project: entry("project"), user: entry("user") });
    try {
      const failed = await f.lifecycle().run("setup", {
        enabled: true,
        confirm: async () => true,
        reload: async () => {
      ...
      assert.deepEqual(
        recovered.participants?.map((participant) => participant.scope),
        ["project", "user"]
      );
      for (const scope of ["project", "user"] as const)
        assert.equal((await json(sidecar(f.paths[scope]))).phase, "committed");
  ...
  await t.test("teardown", async () => {
    ...
    try {
      await setup(f);
      const failed = await f.lifecycle().run("teardown", {
        enabled: false,
        confirm: async () => true,
        reload: async () => {
...
test("controller command frame leaves rejected reload pending until fresh controller startup", async () => {
  const f = await fixture({ project: entry("project"), user: entry("user") });
  try {
    const oldFrame = createPiFffIntegrationController({
      pi: {},
      cwd: f.cwd,
      agentDir: f.agentDir,
    ...
    const freshFrame = createPiFffIntegrationController({
      pi: {},
      cwd: f.cwd,
      agentDir: f.agentDir,
      lifecycle: f.lifecycle(),
...
test("startup finalization checkpoint failures resume idempotently", async (t) => {
  await t.test("setup commit", async () => {
    ...
    try {
      ...
      const interrupted = await f
        .lifecycle({
          checkpoint: (name: string) => {
            if (name === "recovery:setup:journal:project:committed")
  ...
  await t.test("teardown removal", async () => {
    ...
    try {
      await setup(f);
      await f.lifecycle().run("teardown", {
        enabled: false,
        confirm: async () => true,
        reload: async () => {
          throw new Error("reload failed");
      ...
      const interrupted = await f
        .lifecycle({
          checkpoint: (name: string) => {
            if (name === "recovery:teardown:journal:project:removed")
              throw new Error("checkpoint");
          },
        })
        .initialize(false);
...
test("preflight failure, duplicate ambiguity, and cancellation change no bytes", async (t) => {
  await t.test("shadowed preflight failure", async () => {
    ...
    try {
      ...
      const lifecycle = createPiFffLifecycle({
        cwd: f.cwd,
        agentDir: f.agentDir,
        preflight: async (p) => {
          if (p.scope === "user") {
            f.preflighted.push(p.scope);
            throw new Error("invalid shadow");
          }
          return f.preflight(p);
        },
      });
      const result = await lifecycle.run("setup", {
        enabled: true,
        confirm: async () => true,
      ...
      assert.deepEqual(f.preflighted, ["project", "user"]);
      assert.deepEqual(
        await Promise.all([readFile(f.paths.project), readFile(f.paths.user)]),
        before
      );
    } finally {
      await rm(f.root, { recursive: true, force: true });
  ...
  await t.test("trivial plan-shaped fake", async () => {
    ...
    try {
      ...
      const lifecycle = createPiFffLifecycle({
        ...
        agentDir: f.agentDir,
        preflight: async (p) =>
          ({ scope: p.scope, packageRoot: p.packageRoot }) as any,
      });
      const result = await lifecycle.run("setup", {
        enabled: true,
        confirm: async () => true,
  ...
  await t.test("duplicates", async () => {
    const duplicate = {
      packages: ["npm:pi-fff", { source: "npm:pi-fff@0.1.12", extensions: [] }],
      sibling: true,
    };
    const f = await fixture();
    try {
      await writeFile(f.paths.user, JSON.stringify(duplicate));
      const before = await readFile(f.paths.user);
      const result = await f
        .lifecycle()
        .run("setup", {
...
test("prefiltered and string states are journaled truthfully; setup and teardown are idempotent and guarded", async () => {
  ...
  try {
    ...
    const again = await setup(f);
    assert.equal(again.result.outcome, "idempotent");
    assert.equal(again.reloads, 0);
    let reloads = 0;
    const teardown = await f.lifecycle().run("teardown", {
      enabled: false,
      confirm: async () => true,
      reload: async () => {
        reloads++;
    ...
    assert.equal(teardown.outcome, "teardown-committed");
    assert.equal(reloads, 1);
...
test("teardown cancellation and drift preserve both scopes and do not need package files", async () => {
  ...
  try {
    ...
    const drift = await json(f.paths.user);
    drift.packages[0].later = true;
    await writeFile(f.paths.user, JSON.stringify(drift));
    const beforeDrift = await Promise.all([
      readFile(f.paths.project),
      readFile(f.paths.user),
    ]);
    result = await f
    ...
    assert.equal(result.code, "PIFFF_SETTINGS_DRIFT");
    assert.deepEqual(
      await Promise.all([readFile(f.paths.project), readFile(f.paths.user)]),
      beforeDrift
    );
    delete drift.packages[0].later;
    await writeFile(f.paths.user, JSON.stringify(drift));
    result = await f
      .lifecycle()
      .run("teardown", {
...
test("startup recovery rolls setup back while disabled and reports one manual reload", async () => {
  const f = await fixture({ project: entry("project"), user: entry("user") });
  try {
    const interrupted = f.lifecycle({
      checkpoint: (name: string) => {
        if (name === "setup:settings:project:written") throw new Error("crash");
      },
    });
    const failed = await interrupted.run("setup", {
      enabled: true,
      confirm: async () => true,
      reload: async () => assert.fail("reload"),
    });
    ...
    assert.deepEqual(
      (await json(f.paths.project)).packages[0],
      entry("project")
    );
...
test("orphan sidecars remain discoverable with exact settings and sidecar manual paths", async () => {
  ...
  try {
    await f
      .lifecycle({
        checkpoint: (name: string) => {
          if (name === "setup:journal:user:prepared") throw new Error("crash");
        },
      })
      .run("setup", {
        enabled: true,
    ...
    assert.deepEqual(recovered.manualPaths, [
      f.paths.project,
      sidecar(f.paths.project),
      f.paths.user,
      sidecar(f.paths.user),
    ]);
  } finally {
    await rm(f.root, { recursive: true, force: true });
  }
...
test("unsafe startup recovery fails closed with exact manual paths", async () => {
  ...
  try {
    await f
      .lifecycle({
        checkpoint: (name: string) => {
          if (name === "setup:settings:project:written")
    ...
    const changed = await json(f.paths.project);
    changed.packages[0].drift = true;
    await writeFile(f.paths.project, JSON.stringify(changed));
    const before = await Promise.all([
      readFile(f.paths.project),
      readFile(f.paths.user),
    ]);
    ...
    assert.deepEqual(result.manualPaths, [f.paths.project, f.paths.user]);
    assert.deepEqual(
      await Promise.all([readFile(f.paths.project), readFile(f.paths.user)]),
      before
    );
...
test("interrupted teardown rolls back managed state; fully restored teardown only retires journals", async () => {
  for (const point of [
    "teardown:settings:project:written",
    "teardown:journal:project:removed",
  ] as const) {
    const f = await fixture({ project: entry("project"), user: entry("user") });
    try {
      await setup(f);
      ...
      assert.equal(
        recovered.outcome,
        point.includes("settings") ? "recovery-reload-required" : "ready"
      );
      if (point.includes("settings")) {
        assert.deepEqual(
          (await json(f.paths.project)).packages[0].extensions,
          []
        );
...
test("reload capability is required before mutation and committed setup survives disabled startup", async () => {
  ...
  try {
    ...
    const unavailable = await f
      .lifecycle()
      .run("setup", { enabled: true, confirm: async () => true });
    ...
    await f
      .lifecycle({
        checkpoint: (name: string) => {
          if (name === "setup:journal:user:committed") throw new Error("crash");
        },
      })
      .run("setup", {
        enabled: true,
...
test("settings symlinks preserve links, canonical targets, modes, and temp cleanup", async () => {
  ...
  try {
    const before = await lstat(f.paths.project);
    ...
    assert.equal(
      (await readdir(dirname(canonical))).some((name) =>
        name.includes(".tmp-")
      ),
      false
    );
...
test("external settings symlinks are ignored without pi-fff and rejected with it", async () => {
  ...
  try {
    ...
    await writeFile(
      outside,
      JSON.stringify({ packages: ["npm:not-fff@1.0.0"] }) + "\n"
    );
    ...
    const bytes =
      JSON.stringify({ packages: [entry("project")], sibling: true }) + "\n";
...
test("failure injection at every journal/write/phase boundary remains recoverable and cleanup-safe", async () => {
  const points = [
    "setup:journal:project:prepared",
    "setup:journal:user:prepared",
    "setup:settings:project:written",
    "setup:journal:project:settings-written",
    "setup:settings:user:written",
    "setup:journal:user:settings-written",
    "setup:journal:project:reload-pending",
    "setup:journal:user:reload-pending",
    "setup:journal:project:committed",
    "setup:journal:user:committed",
  ];
  for (const point of points) {
    ...
    try {
      ...
      assert.ok(
        ["recovery-reload-required", "ready"].includes(recovery.outcome),
        point
      );
      const committed =
        points.indexOf(point) >= points.indexOf("setup:settings:user:written");
      assert.deepEqual(
        (await json(f.paths.project)).packages[0],
        committed ? { ...entry("project"), extensions: [] } : entry("project"),
        point
      );
      assert.deepEqual(
        (await json(f.paths.user)).packages[0],
        committed ? { ...entry("user"), extensions: [] } : entry("user"),
        point
      );
      for (const dir of [dirname(f.paths.project), dirname(f.paths.user)])
        assert.equal(
...
test("teardown failure injection rolls partial restoration back and retires complete restoration", async () => {
  const points = [
    "teardown:journal:project:prepared",
    "teardown:journal:user:prepared",
    "teardown:settings:project:written",
    "teardown:journal:project:restored",
    "teardown:settings:user:written",
    "teardown:journal:user:restored",
    "teardown:journal:project:reload-pending",
    "teardown:journal:user:reload-pending",
    "teardown:journal:project:removed",
    "teardown:journal:user:removed",
  ];
  for (const point of points) {
    const f = await fixture({ project: entry("project"), user: entry("user") });
    try {
      ...
      const restorationComplete =
        ...
        points.indexOf("teardown:settings:user:written");
      for (const scope of ["project", "user"] as const) {
        ...
        assert.deepEqual(
          ...
          restorationComplete
            ? entry(scope)
            : { ...entry(scope), extensions: [] },
          point
        );
        assert.equal(
          (await readdir(dirname(f.paths[scope]))).some((name) =>
            name.includes(".tmp-")
          ),
          false,
          point
