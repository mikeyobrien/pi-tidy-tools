...
export const VALIDATED_MEDIA_TYPES = ["text/plain"];

/** The public composer sends one base64 upload, never a filesystem path. */
export function decodeArtifactUploads(value: unknown): ArtifactUpload[] {
  ...
  return value.map((item) => {
    ...
    return {
      name: (item.name as string | undefined) ?? "attachment.txt",
